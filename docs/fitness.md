...
- Public reads and the authenticated import: `src/app/fitness_api.rs`
  (routes) over `src/fitness/` (filters, import validation, in-memory
  snapshot, store) and `src/data/fitness.rs` (Postgres). Records are derived
  in `src/records.rs` at snapshot build — there is deliberately no records
  table and no records field in the import payload.
...
(`src/fitness/snapshot.rs`), rebuilt when the fitness version changes. Filter
