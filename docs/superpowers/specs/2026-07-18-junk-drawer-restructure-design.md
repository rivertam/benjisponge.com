...
top-level URL per page, one registry, tags (**Fun** and **Work**) as
...
- Array order = display order within each tag's homepage section; a page
  tagged with both appears in both sections.
- `drawer_page(slug)` lookup helper replaces `interest(slug)`, same
  panic-on-unknown-slug contract (snapshot run surfaces typos).
- Contents: the 8 hobby pages (tags `[Fun]`, no date), `pesky-code`
  (`[Fun]`, dated), `how-bad-are-planes` (`[Fun]`, dated). Resume stays a fixed page, listed in the
...
2. **Fun** section: all Fun-tagged pages; dated entries show the date in
   the rail stamp, undated entries leave it empty.
...
- Data-invariant tests re-point at `drawer.rs`: unique slugs, non-empty
  fields, every page has at least one tag (and no duplicate tags), slugs don't collide with fixed routes (`resume`), every registry
...
- Tags beyond Fun/Work, or retagging pages beyond what's specified.
